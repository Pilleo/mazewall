...
class BpfFilterTest {
    ...
    fun `filter starts with arch check`() {
        ...
        // JEQ arch_audit, skip 1
        assertEquals(0x15.toShort(), filter[1].code)
        assertEquals(1, filter[1].jt.toInt())
        assertEquals(0, filter[1].jf.toInt())
    ...
    fun `filter contains syscall nr load`() {
        val filter = BpfFilter.build(arch, Policy.builder().build())
        // Index 3: Load syscall NR
        assertEquals(0x20.toShort(), filter[3].code)
        assertEquals(0, filter[3].k)
    }

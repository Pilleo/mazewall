...
class BranchRebaserTest {
    ...
    fun `test successful rebase via cherry-pick`() {
        ...
        val rebaser = BranchRebaser(
            ...
            executeInDirNoRetry = { dir, args ->
                ...
                if (cmd.contains("rev-list --reverse")) return@BranchRebaser "commit_1\ncommit_2"
                if (cmd.contains("rev-list --merges")) return@BranchRebaser ""
                if (cmd.contains("--format=%P")) return@BranchRebaser "parent_0"
                if (cmd.contains("--format=%ae")) return@BranchRebaser "161369871+google-labs-jules[bot]@users.noreply.github.com"
    ...
    fun `test merge commits and orchestrator sync commits are skipped`() {
        ...
        val rebaser = BranchRebaser(
            ...
            executeInDirNoRetry = { _, args ->
                val cmd = args.joinToString(" ")
                if (cmd.contains("merge-base")) return@BranchRebaser "sha_base_000"
                if (cmd.contains("rev-list --reverse")) return@BranchRebaser "commit_merge\ncommit_sync\ncommit_good"
                if (cmd.contains("rev-list --merges")) return@BranchRebaser ""
                ...
                if (cmd.contains("--format=%ae commit_good")) return@BranchRebaser "161369871+google-labs-jules[bot]@users.noreply.github.com"
                if (cmd.contains("--format=%ae commit_merge")) return@BranchRebaser "161369871+google-labs-jules[bot]@users.noreply.github.com"
                if (cmd.contains("--format=%ae commit_sync")) return@BranchRebaser "161369871+google-labs-jules[bot]@users.noreply.github.com"
    ...
    @Test
    fun `test cherry pick conflict fails fast`() {
        val commands = mutableListOf<String>()
        ...
        val rebaser = BranchRebaser(
            ...
            executeInDirNoRetry = { _, args ->
                ...
                commands.add("inDirNoRetry: $cmd")
                if (cmd.contains("merge-base")) return@BranchRebaser "sha_base_000"
                if (cmd.contains("rev-list --reverse")) return@BranchRebaser "commit_dirt"
                if (cmd.contains("rev-list --merges")) return@BranchRebaser ""
                if (cmd.contains("--format=%P")) return@BranchRebaser "parent_0"
                if (cmd.contains("--format=%ae")) return@BranchRebaser "161369871+google-labs-jules[bot]@users.noreply.github.com"
                if (cmd.contains("--format=%B")) return@BranchRebaser "feat: agent work"
    ...
    @Test
    fun `test unexpected merge commits fail verification`() {
        val rebaser = BranchRebaser(
            execute = { args ->
                val cmd = args.joinToString(" ")
                if (cmd.contains("headRefName")) return@BranchRebaser "test-branch"
            ...
            executeInDirNoRetry = { _, args ->
                ...
                if (cmd.contains("rev-list --merges")) return@BranchRebaser "unexpected_merge_sha" // Simulate failure
                ""
            },
            clearPrCache = {}
        )

        val worktreeDir = java.io.File("build/tmp/temp-rebase-888")
        worktreeDir.mkdirs()

        val result = try {
            rebaser.run("888", "session_abc")
        } finally {
            worktreeDir.deleteRecursively()
        }
    ...
    @Test
    fun `test fallback rebase uses targetFiles`() {
        val commands = mutableListOf<String>()
        ...
        val worktreeDir = java.io.File("build/tmp/temp-rebase-999")
        worktreeDir.mkdirs()
        val result = rebaser.runFallback("999", "session_abc", listOf("fileA.txt", "fileB.kt"))
        
        assertTrue(result.success)
        assertTrue(commands.contains("inDirNoRetry: git checkout sha_feature_123 -- fileA.txt"))
        assertTrue(commands.contains("inDirNoRetry: git checkout sha_feature_123 -- fileB.kt"))
    }
}

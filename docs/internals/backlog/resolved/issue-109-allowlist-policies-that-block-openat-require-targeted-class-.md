...
**Target:** `AllowListTest.preWarm()`, `designs/enforcer/containment-design.md §3g`
...
**Fix:** Extended `AllowListTest.preWarm()` to touch all `SeccompInstallationState` subclasses before the filter is installed. Added `§3g` to `designs/enforcer/containment-design.md` documenting the rule and its scope.

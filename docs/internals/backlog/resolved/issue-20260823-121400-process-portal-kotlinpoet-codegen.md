...
**Needed:**
1. New `:portal-codegen` Gradle plugin module. Generate host stub that serializes allowed types and attaches `Capability.ReadFd` via `SCM_RIGHTS` into `:portal`. Generate worker dispatcher into `:portal-worker` that never runs in the broker.
...
5. Add `:portal-codegen` and `:portal-worker` to `settings.gradle.kts` and `BacklogValidator.VALID_GRADLE_MODULES` if not already present.

...
*   **Recommendation:** Document the inherent risks of `TSYNC` concurrency in `architecture/security-considerations.md` and recommend applying process-wide policies only during application initialization (e.g. `public static void main`) before extensive multithreading or JIT activity begins.

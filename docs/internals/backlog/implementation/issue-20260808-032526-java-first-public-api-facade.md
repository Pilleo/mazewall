...
1. **Package Placement & Entry Point:** Provide `@file:JvmName("Mazewall")` in root package `io.mazewall` with static factory methods:
   - `Mazewall.builder()` -> `JavaPolicyBuilder`
   - `Mazewall.wrapExecutor(ExecutorService, Policy)` -> `ExecutorService`
   - `Mazewall.installOnCurrentThread(Policy)` -> `InstallationReceipt`
   - `Mazewall.installOnProcess(Policy)` -> `InstallationReceipt`
   - `Mazewall.runContained(Policy, Callable<T>)` -> `T`
2. **Type Ergonomics in Java:** Avoid exposing raw Kotlin phantom generic parameters (`PolicyScope.ThreadLocalSafe`, etc.) in Java method signatures. Expose clean, non-generic `Policy` and `InstallationReceipt` Java interfaces and enforce scope safety validations internally at compilation/installation time.

...
**Root Cause:** Kotlin stdlib collection calls in the per-notification reactor loop ran *before* any classloader check, making it impossible to break the circular dependency.
**Fix:** Introduced `JvmStackInspector` (in `:enforcer`, `io.mazewall.enforcer.supervisor`) as a shared utility implementing a primitive indexed `for` loop over a raw `Array<StackTraceElement>` — zero allocations, zero new class loads. The reactor now obtains the raw stack array and calls `JvmStackInspector.isClassloaderActive(rawStack)` as its *first* action, before any Kotlin stdlib call. If true, the syscall is immediately allowed. Only on the `false` branch (where the ClassLoader lock is not held) does the reactor convert to a `List` and invoke `scopingPolicy.authorize`. Since `:profiler` already depends on `:enforcer`, `JvmStackInspector` is automatically available for reuse in the profiler without any `build.gradle.kts` changes.
**Security Note:** `jdk.internal.reflect.*` was intentionally excluded from the classloader prefix set. Reflection-triggered class loads always flow through `ClassLoader.loadClass()`, so they are caught by the `java.lang.ClassLoader` prefix. Pure `Method.invoke` calls do not hold the `ClassLoader` lock and adding that prefix would create a bypass for any file open during reflective invocation.

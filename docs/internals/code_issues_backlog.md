...
**Non-Performance Downsides of the `ptrace`/`strace` Approach:**
* *Debugger Collisions (Exclusiveness):* A Linux thread can only have a single tracer attached at one time. If Tier P attaches to the JVM via `ptrace`, GDB, LLDB, and IDE debuggers (like IntelliJ) will fail to attach with `EBUSY`, blocking interactive debugging during profiling sessions.
* *JVM Signal Handling Interference:* The JVM relies heavily on system signals for JIT implicit null-pointer checks (`SIGSEGV`) and thread parking/coordination (`SIGUSR1`, `SIGUSR2`). `ptrace` intercepts all signals. The profiler daemon must capture and manually forward them via `PTRACE_CONT`. Any lag or dropped signals during GC safepoints risks causing JVM deadlocks.
* *Thread Cloning Race Conditions:* `ptrace` traces threads at the LWP level. Tracking dynamic JVM carrier thread lifecycles requires capturing `clone`/`clone3` exits using `PTRACE_O_TRACECLONE`. If a thread exits or runs syscalls before the daemon attaches, its system calls will be missed entirely.
* *Argument Inspection/Pointer Resolution:* Syscall registers contain target memory pointers, not direct strings. The daemon must read the JVM process address space at each syscall entry using `process_vm_readv(2)` or `/proc/<pid>/mem`, which is complex and susceptible to TOCTOU memory manipulation races.
* *Process Orphanage/Stops:* If the profiler daemon crashes, the target JVM remains stuck in `TASK_TRACED` (frozen state) until manually killed with `SIGKILL`. Likewise, JVM crashes can leave orphaned tracing processes, requiring strict handshake pipe cleanups.

...
4.  **Hardware-Level CFI:** Because native images are static ELF binaries, they are compatible with hardware-level **Control Flow Integrity (CFI)** features like Intel CET or ARM BTI/PAC (see [Intel CET & Hardware Security Integration](file:///home/leanid/Documents/code/java/jseccomp/docs/internals/intel-cet-jvm-sandbox-integration.md)). These prevent ROP/JOP gadget chains from being executed, which is much harder to enforce in a dynamic JIT environment where code is constantly moving.

...
*   **AOT (Ahead-Of-Time) Sandboxing:** AOT compilation (such as GraalVM Native Image) compiles the application to a static binary prior to execution. When the selected runtime and native dependencies do not require executable mappings after startup, this can support a strict **W^X (Write XOR Execute)** policy that denies later executable-memory creation. Closed-world analysis and dead-code elimination also remove code that the compiler determines is unreachable. This reduces the available high-level capability and gadget surface, but it does not prove that the remaining program is safe: reachability depends on the configured entry points and metadata for reflection, JNI, FFM, resources, serialization, service providers, and other dynamic features. The generated image must be audited together with those inputs and its native dependencies.
...
1.  **Reducing the "Trivial" Bypass Surface:** The thread-hopping bypass is trivial in HotSpot when an attacker can invoke concurrency APIs such as `CompletableFuture.runAsync`. Native Image removes general JIT compilation and ordinary runtime class loading, so an attacker cannot assume that an arbitrary new Java payload can be introduced. This is attack-surface reduction, not a containment proof: any executor, thread-creation path, callback, method-handle target, or equivalent gadget retained anywhere in the application image remains potentially usable by a compromised thread.
2.  **Reducing the JIT Attack Surface:** A standard JVM requires executable code-cache mappings and later code generation. A Native Image configuration that does not use runtime compilation or native dependencies requiring new executable mappings may support a process-wide policy that denies later `PROT_EXEC` transitions. This removes one shellcode-injection route, but does not prevent code-reuse attacks, direct syscalls from existing executable code, or compromise of writable control data.
3.  **Static Reachability as Capability Reduction:** If closed-world analysis removes `ProcessBuilder`, networking implementations, executor paths, or other sensitive facilities from the complete image, Java-level gadget availability is reduced. The result is conditional on the compiler's reachability model and all supplied dynamic-feature metadata. It is not a mathematical guarantee against RCE: native memory corruption can reuse remaining machine-code gadgets or issue syscalls directly, and all threads still share the same process memory.
4.  **Hardware-Assisted Control-Flow Hardening:** Static native images can be candidates for mechanisms such as Intel CET or ARM BTI/PAC when the compiler, linker, runtime, loaded native code, CPU, and kernel all support the selected feature. These mechanisms constrain specific return- or branch-oriented techniques; they do not prevent every ROP/JOP chain or establish an isolation boundary. See [Intel CET & Hardware Security Integration](../../intel-cet-jvm-sandbox-integration.md) for the implementation status and remaining validation work.

**Summary:** Native Image can make some Tier 2 bypasses harder by reducing dynamic runtime facilities and eliminating unreachable code. It does not turn Tier 2 into an isolation boundary. DCE is computed for the complete application image, not separately for a contained thread; capabilities retained for an unrestricted application component remain present in the same process.

#### Dead-Code Elimination and Thread-Containment Eligibility

Dead-code elimination can support a **thread-containment eligibility assessment**, but it cannot certify a library as "RCE-safe." RCE safety is a property of the integrated application, compiler configuration, native dependencies, runtime policy, and attacker capabilities—not of a library artifact in isolation.

An assessment may treat the absence of the following reachable facilities as favorable evidence:

*   platform-thread and virtual-thread creation;
*   executor submission, the common fork-join pool, and asynchronous callbacks;
*   process creation, networking, and filesystem APIs;
*   class loading, expression or script evaluation, reflection, method handles, and unsafe deserialization;
*   JNI, FFM, `Unsafe`, dynamic native-library loading, and other direct native escape hatches; and
*   returned objects that retain file descriptors, sockets, executors, callbacks, or other ambient capabilities.

This evidence is necessarily conditional:

1.  **Whole-image scope:** DCE does not create a distinct reachable-code set for each thread. If the main application retains an executor or network client, it remains in the same binary even when the contained library does not call it during normal execution.
2.  **Dynamic-feature configuration:** Reflection, JNI, FFM, resources, service providers, serialization, and framework-generated registrations can retain code outside an ordinary static call graph. The exact Native Image reachability metadata is part of the security input and must be reviewed.
3.  **Reachability is not exploitability:** The absence of a known dangerous API does not prove that malicious input cannot corrupt application state, exhaust CPU or memory, or exploit an unintended path through code that remains reachable.
4.  **Native ACE remains process-wide:** An attacker controlling native execution can use remaining instruction sequences, issue a raw syscall on the compromised thread, or corrupt shared memory used by an unrestricted sibling thread. Thread-scoped Seccomp can block the immediate syscall but cannot protect the shared address space.

A future Mazewall assessment tool may combine whole-image reachability reports, bytecode/native capability scans, observed syscall profiles, fuzzing, and checks for thread creation or executor submission. A successful result should use a bounded label such as `DATA_ONLY_THREAD_ELIGIBLE`, with the assessed entry points, image configuration, dependency versions, and coverage recorded. It must not emit an `RCE_SAFE` claim. Static and dynamic evidence can reject unsuitable candidates; neither constitutes a proof that an unobserved exploit path does not exist.

For an RCE threat model, the preferred architecture is a minimal AOT-compiled worker in a **separate process**, with DCE reducing that worker's complete capability set and process-wide Seccomp, Landlock, namespaces, cgroups, reduced credentials, restricted networking, and secret separation enforcing the residual boundary. Inside the primary application process, DCE is defense in depth only.
...
Two same-process techniques can reduce the attack surface for constrained guest code, but neither should be described as equivalent to an OS process boundary:

1.  **GraalVM Isolates (Managed-Heap Separation):** An isolate can provide a separate managed heap and runtime state within the same native process. This prevents ordinary Java object sharing across the isolate boundary, but native pointers, runtime vulnerabilities, host interfaces, and process-global resources remain part of the threat model unless a verified protection-domain mechanism supplies additional hardware enforcement.
2.  **WebAssembly (Guest-Memory Bounds):** A Wasm runtime validates guest code and bounds guest linear-memory access, while host imports define its external capabilities. This can isolate well-formed guest execution and avoid exposing the JVM classpath directly. A runtime, JIT, native extension, or incorrectly designed host import can still escape those bounds, so Wasm parsing reduces Java deserialization gadget exposure rather than eliminating RCE risk.

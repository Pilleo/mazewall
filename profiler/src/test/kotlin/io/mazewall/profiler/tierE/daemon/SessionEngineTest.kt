...
internal class SessionEngineTest {
    ...
    private class FakeShim : TierEBpfShim {
        ...
        override fun loadObject(bpfObjectPath: String): Long {
            if (failLoad) throw io.mazewall.profiler.tierE.shim.ShimException("loadObject", -1, "bad elf")
            return ++handleSeq
        }
        ...
        override fun attachMarkerUprobe(handle: Long, pid: Int, sharedObjectPath: String) {
            if (failAttachMarker) {
                throw io.mazewall.profiler.tierE.shim.ShimException("attachMarkerUprobe", -22, "no such file")
            }

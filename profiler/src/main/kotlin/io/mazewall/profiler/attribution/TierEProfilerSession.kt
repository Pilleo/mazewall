...
public class TierEProfilerSession(
    ...
    public fun installAndAttach(targetPid: Int, sessionTag: Int, agentLibrary: Path, markerOffset: Long) {
    ...
    private fun SyscallInvocationEvent.toAttribution(): SyscallAttribution {
        ...
        val active = invocationId.takeIf { it != 0uL }?.toLong()?.takeIf { it > 0 }?.let(::InvocationId)
        val status = when {

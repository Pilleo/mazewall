...
public object TierEKernelSmoke {
    ...
    public fun main(args: Array<String>) {
        require(args.size in 4..8) { "usage: <agent.so> <marker-offset-hex|auto> <workload-classpath> <definitions-file> [virtual|platform] [iterations] [force-cpu] [full-stream|unique-stack-syscall]" }
        val agent = Path.of(args[0]).toAbsolutePath()
        val markerOffset = if (args[1] == "auto") ElfSymbolOffset.resolve(agent, "mazewall_stack_marker")
        else args[1].removePrefix("0x").toLong(16)
        ...
        try {
            ...
            println("tier-e-kernel-smoke: markerTransitions=${session.markerTransitionCount()} " +
            ...
            if (args.getOrNull(4) == "virtual") {
                check(profile.syscalls.filter { it.resolutionStatus == ResolutionStatus.RESOLVED }

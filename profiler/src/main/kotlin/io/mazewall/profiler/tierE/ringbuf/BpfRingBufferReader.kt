...
internal class BpfRingBufferReader(
    ...
    private fun map(fd: Int, length: Long, protection: Int, offset: Long): Long {
    ...
    private fun unmap(address: Long, length: Long) {
    ...
    internal companion object {
        ...
        private fun linuxPageSize(): Long {
            val line = Files.readAllLines(Path.of("/proc/self/smaps"))
                .firstOrNull { it.startsWith("KernelPageSize:") }
                ?: error("cannot determine Linux page size from /proc/self/smaps")
            return line.substringAfter(':').trim().substringBefore(' ').toLong() * 1024L
        }
    }
}

...
internal object InvocationStateBtf {
    ...
    fun load(): ByteArray = requireNotNull(InvocationStateBtf::class.java.getResourceAsStream(RESOURCE)) {
    ...
    fun findStructTypeId(bytes: ByteArray, expectedName: String): Int = findTypeId(bytes, expectedName, BTF_KIND_STRUCT)

    private fun findTypeId(bytes: ByteArray, expectedName: String, expectedKind: Int): Int {
    ...
    private fun payloadSize(kind: Int, vlen: Int): Int = when (kind) {
        1, 14, 17 -> 4
        3 -> 12
        4, 5 -> vlen * 12
        6 -> vlen * 8
    ...
    private fun stringAt(buffer: ByteBuffer, start: Int, length: Int, offset: Int): String {

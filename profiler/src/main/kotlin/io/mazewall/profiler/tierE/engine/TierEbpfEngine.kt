...
public class TierEbpfEngine(
    ...
    public fun install(targetTgid: Int, sessionTag: Int, options: TierEOptions = TierEOptions()) {
    ...
    private fun readCounter(index: Int): Long = Arena.ofConfined().use { arena ->
    ...
    public fun attachMarker(targetPid: Int, agentLibrary: Path, markerOffset: Long) {
        ...
        Arena.ofConfined().use { arena ->
            ...
            for (cpu in onlineCpus().take(1)) {
                ...
                ).getOrThrow("perf_event_open(uprobe,cpu=$cpu)").toInt()
                markerPerfFds += fd
    ...
    override fun close() {
        ...
        markerPerfFds.forEach(::closeIfOpen); markerPerfFds.clear()
        closeIfOpen(linkFd); linkFd = -1
        closeIfOpen(progFd); progFd = -1
        closeIfOpen(markerProgFd); markerProgFd = -1
        closeIfOpen(ringFd); ringFd = -1
        closeIfOpen(lossFd); lossFd = -1
        closeIfOpen(sessionFd); sessionFd = -1
        closeIfOpen(coverageEdgeFd); coverageEdgeFd = -1
        closeIfOpen(invocationStorageFd); invocationStorageFd = -1
        closeIfOpen(invocationBtfFd); invocationBtfFd = -1
    ...
    internal fun buildProgram(options: TierEOptions = this.options): List<Insn> {
        ...
        emit(Insn(MOV_IMM, dst = 5, imm = 0)); emit(Insn(STX_W, dst = FP, src = 5, off = -4))
        ...
        ldMap(1, 3); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -4))
        emit(Insn(CALL, imm = H_LOOKUP)); exitIfZero(0)
        emit(Insn(LDX_W, dst = 4, src = 0, off = 4))
        emit(Insn(MOV_REG, dst = 5, src = 8)); emit(Insn(RSH_IMM, dst = 5, imm = 32))
        ...
        emit(Insn(MOV_IMM, dst = 5, imm = 0)); emit(Insn(STX_DW, dst = FP, src = 5, off = -24))
        emit(Insn(STX_DW, dst = FP, src = 5, off = -16)); emit(Insn(STX_W, dst = FP, src = 5, off = -8))
        ldMap(6, 0); emit(Insn(CALL, imm = H_CURRENT_TASK_BTF)); val noTask = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(MOV_REG, dst = 1, src = 6)); emit(Insn(MOV_REG, dst = 2, src = 0))
        emit(Insn(MOV_IMM, dst = 3, imm = 0)); emit(Insn(MOV_IMM, dst = 4, imm = 0))
        emit(Insn(CALL, imm = H_TASK_STORAGE_GET)); val noStorage = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(MOV_REG, dst = 6, src = 0))
        emit(Insn(LDX_DW, dst = 5, src = 6, off = 0))
        emit(Insn(STX_DW, dst = FP, src = 5, off = -24))
        emit(Insn(LDX_DW, dst = 5, src = 6, off = 8)); emit(Insn(STX_DW, dst = FP, src = 5, off = -16))
        emit(Insn(LDX_W, dst = 5, src = 6, off = 16)); emit(Insn(STX_W, dst = FP, src = 5, off = -8))
        ...
        if (options.emissionMode == TierEEmissionMode.UNIQUE_STACK_SYSCALL) {
            ...
            emit(Insn(LDX_DW, dst = 5, src = 9, off = 8)); emit(Insn(STX_W, dst = FP, src = 5, off = -40))
            emit(Insn(LDX_DW, dst = 5, src = FP, off = -24)); emit(Insn(STX_DW, dst = FP, src = 5, off = -56))
            emit(Insn(MOV_IMM, dst = 5, imm = 0)); emit(Insn(STX_DW, dst = FP, src = 5, off = -48))
            emit(Insn(LDX_W, dst = 5, src = FP, off = -40)); emit(Insn(STX_W, dst = FP, src = 5, off = -48))
            ldMap(1, 4); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -56))
            emit(Insn(CALL, imm = H_LOOKUP)); coverageMiss = emit(Insn(JEQ_IMM, dst = 0))
        ...
        ldMap(1, 1); emit(Insn(MOV_IMM, dst = 2, imm = 88)); emit(Insn(MOV_IMM, dst = 3, imm = 0))
        emit(Insn(CALL, imm = H_RB_RESERVE)); val reservationFailed = emit(Insn(JEQ_IMM, dst = 0)); emit(Insn(MOV_REG, dst = 7, src = 0))
        emit(Insn(CALL, imm = H_KTIME)); emit(Insn(STX_DW, dst = 7, src = 0, off = 0))
        emit(Insn(MOV_REG, dst = 5, src = 8)); emit(Insn(RSH_IMM, dst = 5, imm = 32)); emit(Insn(STX_W, dst = 7, src = 5, off = 8))
        emit(Insn(STX_W, dst = 7, src = 8, off = 12))
        emit(Insn(LDX_DW, dst = 5, src = 9, off = 8)); emit(Insn(STX_W, dst = 7, src = 5, off = 16))
        emit(Insn(LDX_W, dst = 5, src = FP, off = -8)); emit(Insn(STX_W, dst = 7, src = 5, off = 20))
        emit(Insn(LDX_DW, dst = 5, src = FP, off = -24)); emit(Insn(STX_DW, dst = 7, src = 5, off = 24))
        emit(Insn(LDX_DW, dst = 5, src = FP, off = -16)); emit(Insn(STX_DW, dst = 7, src = 5, off = 32))
        ...
        listOf(112, 104, 96, 56, 72, 64).forEachIndexed { index, offset ->
            emit(Insn(MOV_REG, dst = 1, src = 7)); emit(Insn(ADD_IMM, dst = 1, imm = 40 + index * 8))
            emit(Insn(MOV_IMM, dst = 2, imm = 8))
            emit(Insn(MOV_REG, dst = 3, src = 6)); emit(Insn(ADD_IMM, dst = 3, imm = offset))
        ...
        if (options.emissionMode == TierEEmissionMode.UNIQUE_STACK_SYSCALL) {
            ...
            ldMap(1, 4); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -56))
            emit(Insn(MOV_IMM, dst = 5, imm = 1)); emit(Insn(STX_DW, dst = FP, src = 5, off = -64))
            emit(Insn(MOV_REG, dst = 3, src = FP)); emit(Insn(ADD_IMM, dst = 3, imm = -64))
            emit(Insn(MOV_IMM, dst = 4, imm = 0)); emit(Insn(CALL, imm = H_UPDATE))
        ...
        emit(Insn(MOV_REG, dst = 1, src = 7)); emit(Insn(MOV_IMM, dst = 2, imm = 0)); emit(Insn(CALL, imm = H_RB_SUBMIT))
        val submitted = emit(Insn(JA))
        val lossHandler = p.size
        ldMap(1, 2); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -4))
        emit(Insn(CALL, imm = H_LOOKUP)); val missingLossCounter = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(MOV_IMM, dst = 5, imm = 1)); emit(Insn(ATOMIC_ADD_DW, dst = 0, src = 5))
        emit(Insn(MOV_IMM, dst = 0, imm = 0)); emit(Insn(EXIT))
    ...
    internal fun buildMarkerProgram(): List<Insn> {
        ...
        fun loadMap(destination: Int, index: Int) {
            emit(Insn(LD_MAP_FD, dst = destination, src = PSEUDO_FD, imm = index))
            emit(Insn(code = 0))
        }

        emit(Insn(MOV_REG, dst = 9, src = 1))
        emit(Insn(CALL, imm = H_PID_TGID)); emit(Insn(MOV_REG, dst = 6, src = 0))
        emit(Insn(MOV_IMM, dst = 5, imm = 0)); emit(Insn(STX_W, dst = FP, src = 5, off = -28))
        loadMap(1, 2); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -28))
        emit(Insn(CALL, imm = H_LOOKUP)); val noTargetConfig = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(LDX_W, dst = 4, src = 0, off = 4))
        emit(Insn(MOV_REG, dst = 5, src = 6)); emit(Insn(RSH_IMM, dst = 5, imm = 32))
        ...
        emit(Insn(MOV_REG, dst = 5, src = 7)); emit(Insn(RSH_IMM, dst = 5, imm = 32))
        emit(Insn(STX_W, dst = FP, src = 5, off = -32))
        emit(Insn(MOV_IMM, dst = 4, imm = 0)); emit(Insn(STX_W, dst = FP, src = 4, off = -28))
        loadMap(1, 2); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -28))
        emit(Insn(CALL, imm = H_LOOKUP)); val noSession = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(LDX_W, dst = 4, src = 0)); emit(Insn(LDX_W, dst = 5, src = FP, off = -32))
        val wrongSession = emit(Insn(JNE_REG, dst = 5, src = 4))
        val acceptedMarker = program.size
        emit(Insn(MOV_IMM, dst = 5, imm = 1)); emit(Insn(STX_W, dst = FP, src = 5, off = -28))
        loadMap(1, 1); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -28))
        emit(Insn(CALL, imm = H_LOOKUP)); val noMarkerCounter = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(MOV_IMM, dst = 5, imm = 1)); emit(Insn(ATOMIC_ADD_DW, dst = 0, src = 5))
        emit(Insn(CALL, imm = H_PID_TGID)); emit(Insn(STX_DW, dst = FP, src = 0, off = -40))
        emit(Insn(MOV_IMM, dst = 5, imm = 2)); emit(Insn(STX_W, dst = FP, src = 5, off = -28))
        loadMap(1, 1); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -28))
        emit(Insn(CALL, imm = H_LOOKUP)); val noPidSlot = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(LDX_DW, dst = 5, src = FP, off = -40)); emit(Insn(STX_DW, dst = 0, src = 5))
        ...
        emit(Insn(MOV_IMM, dst = 5, imm = 3)); emit(Insn(STX_W, dst = FP, src = 5, off = -28))
        loadMap(1, 1); emit(Insn(MOV_REG, dst = 2, src = FP)); emit(Insn(ADD_IMM, dst = 2, imm = -28))
        emit(Insn(CALL, imm = H_LOOKUP)); val noFailureCounter = emit(Insn(JEQ_IMM, dst = 0))
        emit(Insn(MOV_IMM, dst = 5, imm = 1)); emit(Insn(ATOMIC_ADD_DW, dst = 0, src = 5))
    ...
    private fun bpfResult(cmd: Long, attr: MemorySegment, size: Long) = native.syscall(
        SYS_BPF,
    ...
    private fun loadBtf(bytes: ByteArray): Int = Arena.ofConfined().use {
    ...
    private fun loadProg(program: List<Insn>, mapFds: List<Int>, programType: Int, programName: String): Int {
    ...
    private fun onlineCpus(): List<Int> = Files.readString(Path.of("/sys/devices/system/cpu/online"))
        .trim()
        .split(',')
        .flatMap { range ->
    ...
    private fun rawIoctl(fd: Int, request: Long, argument: Long) {
        native.syscall(
            SYS_IOCTL,

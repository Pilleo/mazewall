...
public object TierEDifferentialSmoke {
    ...
    public fun main(args: Array<String>) {
        ...
        try {
            ...
            println("tier-e-differential: tierENs=$tierNanos userNotifNs=$oracleNanos " +
    ...
    private fun runJava(mainClass: String, vararg arguments: String): String {
        val command = listOf(
    ...
    private fun metric(output: String, name: String): Long =
        requireNotNull(Regex("(?:^|\\s)$name=(\\d+)").find(output)?.groupValues?.get(1)?.toLongOrNull()) {

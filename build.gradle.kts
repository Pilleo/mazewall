...
val generateKnowledgeMap by tasks.registering {
    ...
    doLast {
        class KnowledgeMapGenerator(private val rootDir: File) {
            ...
            private val mapsDir = File(docsDir, "designs/core/maps")
            private val backlogDir = File(docsDir, "backlog")
            private val targetMapFile = File(docsDir, "designs/core/architectural-map.md")
            ...
            fun generate() {
                ...
                val designDocs = docsDir.walkTopDown()
                    .filter { it.isFile && it.name.endsWith(".md") && !it.absolutePath.contains("/maps/") && !it.absolutePath.contains("/backlog/") }
                    .toList()
                    .sortedBy { it.name }
            ...
            private fun generateMermaidForScope(scopeName: String, designDocs: List<File>, backlogIssues: List<File>): String? {
                ...
                for (doc in designDocs) {
                    ...
                    if (filename in listOf("designs/core/architectural-map.md", "architectural-map.md", "README.md", "documentation-standards.md", "documentation-standards.md")) {
                        continue
                    }
                    ...
                    val relPath = doc.relativeTo(mapsDir).path
                    clicks.add("    click $docId \"$relPath\"")
                ...
                for (issue in backlogIssues) {
                    ...
                    val relPath = issue.relativeTo(mapsDir).path
                    clicks.add("    click $issueId \"$relPath\"")
